function [new_schedule,flag,f_cost,x_hat,eta_sum] = SubGradientDescent(ev,ev_fun_inputs)
    ...
    for i = 1:size(deltas,2)
        g_vals(i) = ev.schedule'*(diag(deltas(i).pr(:,1))*tot_demand+deltas(i).pr(:,2));
    end
    ...
    g_gradient = 2*A_deltas*ev.schedule + A_deltas*x_meno_i+b_deltas;
    
    gradient = f_gradient+g_gradient;
    ...
    for i = 1:size(deltas,2)
        g_vals(i) = x_hat'*(diag(deltas(i).pr(:,1))*new_tot+deltas(i).pr(:,2));
    end
    
    f_cost = x_hat'*(A_0*new_tot+b_0) + max(g_vals); 
end