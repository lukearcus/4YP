...
for i = 1:N_ag
    ...
    evNR2(i) = clPlayer(i,T,constraint,energy_amount(i),deltaT,efficiency(i),@GradientDescent);
...
for i = 1:N_ag
    coordNR(i) = clCoord(i,T,A_0,N_samples,N_ag,@maxnashgameQP_v3);
    coordNR2(i) = clCoord(i,T,A_0,N_samples,N_ag,@maxnashgameQP_v3);
    coordMM(i) = clCoord(i,T,A_0,N_samples,N_ag,@maxnashgameQP_v3);
    coordW(i) = clCoord(i,T,A_0,N_samples,N_ag,@maxnashgameQP_v3);
end
...
N_tests = 00; %number of tests for checking violation rate
...
clearvars -except coordNR evNR coordNR2 evNR2 coordMM evMM coordW evW samples setup Max_ag samplesRemoval