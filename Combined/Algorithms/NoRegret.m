...
while samplesRemoved <= samplesRemoval.k
    ...
    for itt = 1:stopping.max_itt
        ...
        for i = 1:setup.N_ag
            ...
            costs(i,t,samplesRemoved+1) = ev(i).cost/setup.N_ag;
        ...
        if isequal(ev(1).minimiser,@GradientDescent)
            ...
            coord.maxcentral(coord_fun_input);
        else
            g_vals = zeros(1,size(deltas.simple,2));
            for m = 1:size(deltas.simple,2)
                g_vals(m) = coord.tot_demand'*(deltas.simple(m).pr(:,1).*coord.tot_demand + deltas.simple(m).pr(:,2));
            end
    ...
    if samplesRemoval.mode == 1
        [deltas.simple,reg_y,removed] = maximum_removal(coord,deltas.simple,coord.y_max);
    else
        if samplesRemoved == samplesRemoval.k
            samplesRemoved = samplesRemoved+1;
        end
        [deltas.simple,reg_y,removed] = all_active_removal(coord,deltas.simple,coord.y_max,SamplesRemoval.k-samples);
    end
    ...
    if isequal(ev(1).minimiser,@GradientDescent)
        ...
        coord.y_max = zeros(1,size(deltas.simple,2));
        coord.y_max(1) = 1;
    end
